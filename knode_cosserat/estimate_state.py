...
def estimate_state(data, tensions, robot):
    ...
    for t in range(T):
        ...
        estimated_state[t, 4:7, 0] = 0 # setting the quaternion h1 h2 h3 to 0 for the root, always pointing upwards
...
if __name__ == '__main__':
    ...
    ref_robot = CosseratRod() #3.23
    measured_loc = [0, 3.23, 5.13, 7.07, 9]  # measurement location ratios have big impact on interpolation results
    real_data_partial_state = np.load('data/real_physical/sin_1_0_amp_300.bag.npy', allow_pickle=True).item()
    ...
    np.save('data/real_physical/sin_1_0_amp_300_estimated.npy', 
